...
function sanitizeModelTitle(raw: string): string {
  ...
  line = line.replace(/^(название|тема|урок)\s*[:：—-]\s*/i, '').trim()
  return line
}
...
/**
 * Короткое название урока по переписке: сначала пробуем GigaChat, затем первую реплику пользователя, иначе дефолт.
 */
  ...
  const completionMessages: GigaChatMessage[] = [
    {
      role: 'system',
      content:
        'Ты составляешь заголовок для списка уроков в приложении. По переписке пользователя с ассистентом про языковое обучение придумай короткое название на русском: 3–10 слов, без кавычек, без префиксов вроде «Название:». Отрази тему или язык/уровень, если они явно есть. Одна строка — только заголовок.',
    },
    {
      role: 'user',
      content: `Переписка:\n\n${dialogue}\n\nЗаголовок для карточки урока (одна строка):`,
    },
  ]
